...
execute unless entity @s[scores={ability_charge=1..2}] run summon armor_stand ~ ~ ~ {NoGravity:1b,Invulnerable:1b,Marker:1b,Invisible:1b,NoBasePlate:1b,Tags:["hook_marker"]}
...
execute unless entity @s[scores={ability_charge=2}] run execute as @e[tag=hook_marker,distance=..2,limit=1] at @s run execute positioned ~ ~1 ~ run function cartographer_mob_abilities:charge/hookshot_raycast

execute if entity @s[scores={ability_charge=2}] run kill @e[tag=hook_marker]

execute unless entity @s[scores={ability_charge=1..2}] run tag @s add is_hooking
