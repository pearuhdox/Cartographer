...
execute unless entity @s[scores={ability_charge=2}] run particle minecraft:crit ~ ~1 ~ 0.75 0.75 0.75 0.3 50 normal @a
execute unless entity @s[scores={ability_charge=2}] run particle minecraft:sneeze ~ ~1 ~ 0.5 0.5 0.5 0 5 normal @a

execute unless entity @s[scores={ability_charge=3}] run scoreboard players add @s ability_charge 1

execute if entity @s[scores={ability_charge=3}] run function cartographer_mob_abilities:abilities/trapper_cast