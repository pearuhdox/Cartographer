...
execute unless entity @s[scores={ability_charge=2}] as @e[tag=webshot_marker,distance=..2,limit=1] at @s run scoreboard players set @s helper_raycast 13

execute unless entity @s[scores={ability_charge=2}] as @e[tag=webshot_marker,distance=..2,limit=1] at @s positioned ~ ~1 ~ run function cartographer_mob_abilities:charge/webshot/raycast
