...
summon trident ~ ~1.2 ~ {damage:2.0d,Tags:["new_proj_replace","passive_trident","passive_replaced_projectile"]}
...
execute as @e[tag=passive_trident,tag=new_proj_replace,limit=1,sort=nearest] at @s run function cartographer_mob_abilities:passive/projectile/helper/apply_data_motion

function cartographer_mob_abilities:passive/projectile/helper/remove_other_sounds
playsound minecraft:entity.drowned.shoot hostile @a[distance=..20] ~ ~ ~ 1.5 1.2
