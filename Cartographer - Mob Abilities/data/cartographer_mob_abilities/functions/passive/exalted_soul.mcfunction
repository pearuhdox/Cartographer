...
#execute if entity @s[scores={helper_lifetime=1..30}] run tp @s ~ ~0.5 ~

execute unless entity @s[scores={helper_lifetime=40..}] run tp @s ~ ~0.05 ~ ~2 ~
scoreboard players add @s helper_lifetime 1

execute if entity @s[scores={helper_lifetime=40..}] unless entity @e[type=armor_stand,tag=exalted_weapon,sort=nearest,limit=1,distance=..0.5] anchored eyes facing entity @e[type=armor_stand,tag=exalted_weapon,sort=nearest,limit=1,distance=0.4..] feet positioned ^ ^ ^0.75 rotated as @s positioned ^ ^ ^5 facing entity @s eyes facing ^ ^ ^-1 positioned as @s run tp @s ^ ^ ^0.25 ~ ~
