...
execute as @s[type=#bb:hostile,tag=ca.has_active,tag=!ca.active_when_hitting,tag=!ca.active_when_hit,tag=can_see_player,tag=!tokened,tag=priority_tokened] at @s if entity @a[distance=..30] unless score @s cooldown matches 4.. run function cartographer_mob_abilities:helper/token/take
execute as @s[type=#bb:hostile,tag=ca.has_active,tag=!ca.active_when_hitting,tag=!ca.active_when_hit,tag=can_see_player,tag=!tokened] at @s if entity @a[distance=..30] unless score @s cooldown matches 4.. run function cartographer_mob_abilities:helper/token/take
