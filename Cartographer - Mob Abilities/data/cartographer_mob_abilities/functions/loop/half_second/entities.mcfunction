...
#Run all actives
execute if entity @s[tag=avail_target] as @s[type=#cartographer_core:hostile,tag=ca.has_active,tag=attacking,scores={cooldown=0}] at @s run function cartographer_mob_abilities:loop/half_second/run_actives
execute unless entity @s[tag=avail_target] as @s[type=#cartographer_core:hostile,tag=ca.has_active,tag=attacking,scores={cooldown=0}] at @s run function cartographer_mob_abilities:loop/half_second/cancel_actives
