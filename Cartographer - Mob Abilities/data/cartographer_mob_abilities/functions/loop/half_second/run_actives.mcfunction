...
#Smash
#execute if entity @s[tag=ca.smash,tag=tokened,scores={cooldown=0}] unless entity @s[scores={recast_count=1..}] if entity @a[gamemode=!spectator,gamemode=!creative,distance=..15] run function cartographer_mob_abilities:charge/smash
...
execute if entity @s[tag=ca.smash,tag=tokened,scores={cooldown=0}] unless entity @a[gamemode=!spectator,gamemode=!creative,distance=..12] run function cartographer_mob_abilities:helper/token/cancel_ability
