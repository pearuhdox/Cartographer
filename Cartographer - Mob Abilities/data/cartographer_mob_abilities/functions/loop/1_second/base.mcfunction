...
execute as @a[tag=!no_near_mobs] at @s unless entity @e[type=#cartographer_core:hostile,distance=..30] run function cartographer_mob_abilities:loop/1_second/out_combat_tag
...
execute unless score $hook_check ca.hooked matches 1.. if score $tokened_total ca.var matches 1.. as @e[type=#cartographer_core:hostile,tag=ca.has_active,tag=tokened,tag=avail_target,tag=!attacking,scores={cooldown=0},limit=1,sort=random] at @s run tag @s add attacking
execute if score $hook_check ca.hooked matches 1.. if score $tokened_total ca.var matches 1.. as @e[type=#cartographer_core:hostile,tag=ca.has_active,tag=!hookshot,tag=tokened,tag=avail_target,tag=!attacking,scores={cooldown=0},limit=1,sort=random] at @s run tag @s add attacking
