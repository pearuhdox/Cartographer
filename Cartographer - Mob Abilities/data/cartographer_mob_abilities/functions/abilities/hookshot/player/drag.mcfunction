execute unless score @s ca.delta.cooldown matches 1.. if entity @s[tag=ca.faster_hook_drag] run scoreboard players set $strength delta.api.launch 4000
execute unless score @s ca.delta.cooldown matches 1.. unless entity @s[tag=ca.faster_hook_drag] run scoreboard players set $strength delta.api.launch 2750
