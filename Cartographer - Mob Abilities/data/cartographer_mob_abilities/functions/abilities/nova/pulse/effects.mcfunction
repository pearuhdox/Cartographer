execute as @a[gamemode=!spectator,gamemode=!creative,distance=..4.5] at @s run scoreboard players operation @s cdl.damage_queue = $damage ca.ability_dmg
execute as @a[gamemode=!spectator,gamemode=!creative,distance=..4.5] at @s unless entity @s[tag=no_cdl_msg] run scoreboard players set @s cdl.death_id 310203
execute as @a[gamemode=!spectator,gamemode=!creative,distance=..4.5] at @s run tag @s remove no_cdl_msg
execute as @a[gamemode=!spectator,gamemode=!creative,distance=..4.5] at @s run tag @s add ca.pull
execute rotated ~ 0 positioned ^ ^ ^1 facing entity @s feet rotated ~ 0 positioned as @s as @a[gamemode=!spectator,gamemode=!creative,distance=..4.5] positioned as @s run function cartographer_core:helper/damage_knockback/targeting_direction
