...
execute if score $alchemist ca.trap_traits matches 1 run function cartographer_mob_abilities:abilities/trapper/cloud/alchemist_copy

scoreboard players operation $damage ca.trap_traits = @s ca.ability_dmg

execute as @e[type=area_effect_cloud,tag=ca.new_trap_cloud,limit=1,sort=nearest] at @s run function cartographer_mob_abilities:abilities/trapper/cloud/setup