execute unless entity @s[tag=ca.ability_dmg+++] unless entity @s[tag=ca.ability_dmg---] unless entity @s[tag=ca.ability_dmg+] unless entity @s[tag=ca.ability_dmg++] unless entity @s[tag=ca.ability_dmg-] unless entity @s[tag=ca.ability_dmg--] run summon creeper ~ ~ ~ {Tags:["spawn_checked"],NoGravity:1b,Silent:1b,ExplosionRadius:2b,Fuse:1,ignited:1b,CustomName:'{"text":"volatile magic"}'}
