...
execute if entity @s[tag=ca.has_kited] unless entity @s[tag=ca.spin_shot] unless entity @s[tag=ca.volley] unless entity @s[tag=ca.smash] unless entity @s[tag=ca.laser] unless entity @s[tag=ca.haunt] unless entity @s[tag=ca.sniper_shot] if score @s cooldown matches 0 run scoreboard players remove @s ability_charge 1