...
execute if block ~ ~-0.2 ~ trapped_chest run summon item_frame ~ ~-1 ~ {Tags:["mimic_position"],Item:{}}
...
execute if entity @e[type=item_frame,tag=mimic_position] run tellraw @s {"text":"The trapped chest you were standing on is now a mimic!","color":"aqua","italic":false}
