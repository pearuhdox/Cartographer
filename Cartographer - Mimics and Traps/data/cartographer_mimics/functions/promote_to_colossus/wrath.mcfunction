...
data modify storage cartographer_mimics:colossus data.colossus_type set value "wrath"

data modify storage cartographer_mimics:colossus registry append from storage cartographer_mimics:colossus data

execute if block ~ ~-0.2 ~ spawner run tellraw @p {"text":"The spawner above is now a Colossus! (Wrath)","color":"aqua","italic":false}
