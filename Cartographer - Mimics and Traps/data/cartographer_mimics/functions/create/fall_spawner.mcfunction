...
data modify storage cartographer_mimics:colossus data.type set value "falling"
...
execute if block ~ ~-0.2 ~ spawner if entity @e[type=block_display,tag=ca.trap_spawner,limit=1,sort=nearest,distance=..2] run tellraw @p [{"text":"This trap spawner is now an Falling Spawner.","color":"aqua","italic":false}]