...
execute if block ~ ~-0.2 ~ spawner if entity @e[type=block_display,tag=ca.trap_spawner,limit=1,sort=nearest,distance=..2] run tellraw @s [{"text":"The Trap Spawner you are standing on is now an Falling Spawner.","color":"aqua","italic":false}]