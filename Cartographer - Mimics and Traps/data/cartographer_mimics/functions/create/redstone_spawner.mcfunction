...
execute if block ~ ~-0.2 ~ spawner if entity @e[type=block_display,tag=ca.trap_spawner,limit=1,sort=nearest,distance=..2] run tellraw @p [{"text":"The Trap Spawner you are standing on is now a Redstone Spawner.","color":"aqua","italic":false}]