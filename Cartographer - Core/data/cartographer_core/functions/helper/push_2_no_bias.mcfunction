########################################
# File: Cartographer Core - Helper / Push 2
#
# Author: PearUhDox | Date: 1/11/2021
#
# Description: Based on a derived vector, pushes the mob opposite its facing direction.
# The push is double the magnitude of Push.
########################################

execute as @s at @s run tp @e[type=armor_stand,tag=vector] ^ ^ ^2
execute as @e[type=armor_stand,tag=vector,limit=1,sort=nearest] at @s run tp @s ~ ~-0.15 ~
