...
execute positioned ~ ~ ~ as @e[type=#cartographer_core:hostile,tag=!thrusted,dx=0,nbt={HurtTime:0s}] positioned ~-0.59 ~-0.59 ~-0.59 if entity @s[dx=0] at @s run function cartographer_custom_enchantments:enchant_effects/surging_strike_effects

#Run the raycast again.
execute if score @s helper_raycast matches 1.. positioned ^ ^ ^0.5 run function cartographer_custom_enchantments:enchant_effects/surging_strike_raycast