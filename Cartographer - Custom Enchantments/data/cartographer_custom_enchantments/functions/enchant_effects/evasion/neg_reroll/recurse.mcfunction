...
execute if score @s ca.evade_chance > $percent_evade ca.evade_chance run tag @s remove evading

execute if entity @s[tag=evading] run say Test for Failure - Passed Evasion
execute unless entity @s[tag=evading] run say Test for Failure - Failed Evasion

scoreboard players remove $chances ca.luck- 1
execute if entity @s[tag=evading] if score $chances ca.luck- matches 1.. run function cartographer_custom_enchantments:enchant_effects/evasion/neg_reroll/recurse