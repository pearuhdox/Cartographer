...
execute if score $ranged ca.melee_chance <= $percent_poss ca.melee_chance unless score @s ca.possess_time matches 1.. at @s anchored eyes run particle minecraft:glow_squid_ink ~ ~1 ~ 0 0.5 0.3 0.1 5 normal
execute if score $ranged ca.melee_chance <= $percent_poss ca.melee_chance unless score @s ca.possess_time matches 1.. at @s run playsound minecraft:entity.skeleton_horse.hurt player @a[distance=..16] ~ ~ ~ 1 0.5
