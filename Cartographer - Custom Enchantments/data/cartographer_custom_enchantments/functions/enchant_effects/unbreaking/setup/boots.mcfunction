...
data modify storage cartographer_custom_enchantments:unbreaking data set from entity @s Inventory[{Slot:100b}]
...
scoreboard players operation $dur ca.unbreaking = @s ca.unb_feet

function cartographer_custom_enchantments:enchant_effects/unbreaking/do_changes

execute if score $unb_succeed ca.ench_var matches 1.. run loot replace entity @s armor.feet 1 mine 4206900 0 4206900 air{drop_contents:1b}
