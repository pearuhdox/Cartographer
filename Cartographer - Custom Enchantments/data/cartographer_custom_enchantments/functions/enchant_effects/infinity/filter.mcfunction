execute store result score $slot i.var run data get storage i:internal temp[0].Slot
execute store result score $pass i.var if score $slot i.var matches 0..8
execute if score $pass i.var matches 1 run data modify storage i:internal working_it append from storage i:internal temp[0]
