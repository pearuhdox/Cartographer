execute store result storage cartographer_custom_enchantments:loyalty_inventory Temp.tag.DurationRemaining int 1 run scoreboard players add $duration ca.loyalty 0 
...
execute if score $slot ca.loyalty matches -106.. in minecraft:overworld run data modify block 4206900 1 4206900 Items append from storage cartographer_custom_enchantments:loyalty_inventory Temp
...
scoreboard players set $more_waiting ca.loyalty 1

particle minecraft:splash ~ ~0.2 ~ 0.2 0 0.2 0 10 normal